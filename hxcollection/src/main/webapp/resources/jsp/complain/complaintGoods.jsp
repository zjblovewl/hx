    ...
    <div class="customsearch allLine" id="query-form">   		
     	<div class="customsearch-input">
	     	<span class="label-select-oneline">举报类型：</span>
	     		<select name="reportType" class="form-control select-style">	
     	...
     	<div class="customsearch-input">
     		<span class="label-input-oneline">举报内容：</span>
		    <input name="reportContent" placeholder='内容' type="text" class="form-control">
     	</div>          
...
<script>
    var queryUrl = '<%=basePath%>goodsComplaint/queryGoodsComplaintList.do?rnd=' + Math.random();
    var deleteByIdUrl =  '<%=basePath%>goodsComplaint/deleteGCById.do?rnd=' + Math.random();
    var deleteBatchUrl = '<%=basePath%>goodsComplaint/delGCBatch.do?rnd=' + Math.random();
    ...
    var columnsArray = [{
        ...
        width: 150,
        formatter :function(){       	
        	return "<i class='fa fa-file-text' title='投诉处理'  style='margin-right:8px;font-size: 16px;'></i>"+
    ...
    function complaint(row){
		...
		modalObj.modalOut("complain/complaintGoodsEdit.jsp",{edit:"complain",id:row.id});		
    }
    //删除
    function deleID(id){
		modalObj.$.showConfirm("确认删除当前条目?", function(){
			$.post(deleteByIdUrl,{gcIds:id});
