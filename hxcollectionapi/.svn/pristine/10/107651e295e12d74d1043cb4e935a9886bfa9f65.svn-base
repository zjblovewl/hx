...
public class UserServiceImpl extends BaseLogger implements UserService {
	...
	public ResponseParamVo login(RequestParamVo vo) {
		...
		String version = (String) vo.getHeader().getVersion();
		String deviceInfo = (String) vo.getHeader().getDevice_info();
		...
		if(userMap!=null && userMap.size() > 0)
		{
			try {
				...
				loginLogParamMap.put("user_id", (String)userMap.get("user_id"));
				loginLogParamMap.put("phone", (String)userMap.get("phone"));
				loginLogParamMap.put("nick_name", (String)userMap.get("nick_name"));
				userDao.insertLoginLog(loginLogParamMap);
				//1、根据其来源，存储相对应的Token
				String isSource = vo.getHeader().getIs_source();
				String tokenkey = Configuration.getInstance().getValue("tokenkey");
				if(isSource.equals("1"))
				{
					Token = String.valueOf(userMap.get("user_id")) + "MOBILE" + String.valueOf(System.currentTimeMillis()) + tokenkey;
					Token = Des3.encode(Token);
					redisTemplate.opsForValue().set(userMap.get("user_id") + "MobileToken",Token);
				}

				if(isSource.equals("2"))//PC端
				{
					Token  = String.valueOf(userMap.get("user_id")) + "PC" + String.valueOf(System.currentTimeMillis()) + tokenkey;
					Token = Des3.encode(Token);
					redisTemplate.opsForValue().set(userMap.get("user_id") + "PcToken",Token,baseConstant.getExpireTime(), TimeUnit.MINUTES);
				}
			} catch (Exception e) {
			...
			return StringUtil.returnResponseVo(baseConstant.getRightCode(),"登录成功",Token,String.valueOf(userMap.get("user_id")),userMap);
		}
		else
		{
	...
	@Override
	public ResponseParamVo thirdLogin(RequestParamVo vo) {
		Map<String,Object> bodyMap = (Map<String, Object>) vo.getBody();
		Map<String,String> paramMap =new HashMap<String, String>();
		Map<String,String> loginLogParamMap =new HashMap<String, String>();
		String nick_name = bodyMap.get("nick_name").toString();
		String third_id = bodyMap.get("third_id").toString();
		String third_type = bodyMap.get("third_type").toString();
		paramMap.put("nick_name", nick_name);
		paramMap.put("third_id", third_id);
		paramMap.put("third_type", third_type); // 1：微信  2：QQ 3：支付宝  4：微博
		
		String version = (String) vo.getHeader().getVersion();
		String deviceInfo = (String) vo.getHeader().getDevice_info();

		
		int count = userDao.selectCountByThirdId(paramMap);
		if(count == 0)
		{
			paramMap.put("uuId", NumberUtil.getSysJournalNo(32, false));
			userDao.insertRegisterUser(paramMap);
		}
		Map<String, Object> userMap=null;
		try {
			userMap = userDao.thirdLogin(paramMap);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		String Token = "";
	 ...
	 * @date: 2018年5月25日 上午10:42:19
	 * @author:chenchen
	 */
	public ResponseParamVo logout(RequestParamVo vo) {				
		try {
			Map<String,Object> bodyMap = (Map<String, Object>) vo.getHeader();			
			redisTemplate.delete(bodyMap.get("token"));
			return StringUtil.returnResponseVo(baseConstant.getRightCode(),"登出成功","","",new JSONObject());
		} catch (Exception e) {
			log.info("登出异常",e.getMessage());
			e.printStackTrace();
			return StringUtil.returnResponseVo(baseConstant.getErrorCode(),"登出异常","","",new JSONObject());
		}
	}
	...
	public ResponseParamVo getUserList(RequestParamVo vo) {
		...
		Map<String,String> paramMap =new HashMap<String, String>();
		try {
			Integer pageNumber = Integer.parseInt(bodyMap.get("page_num").toString());
			Integer pageSize = Integer.parseInt(bodyMap.get("page_size").toString());
	...
	public ResponseParamVo registerUser(RequestParamVo vo) {
		...
		try {
			...
			userParamMap.put("nick_name", phone);
	...
	public ResponseParamVo updateUserInfo(RequestParamVo vo) {
		...
		try {
			...
			userParamMap.put("consignee_name", bodyMap.get("consignee_name").toString());
			userParamMap.put("consignee_phone", bodyMap.get("consignee_phone").toString());
			userParamMap.put("consignee_address", bodyMap.get("consignee_address").toString());
			//更新图片信息
...
//			paramsMap.put("processingState", processingState);
	...
	public ResponseParamVo realnameAuthentication(RequestParamVo vo) {
	 ...
	 /**第三方绑定手机号**/
	@Override
	public ResponseParamVo thirdBindPhone(RequestParamVo vo) {
		Map<String,Object> bodyMap = (Map<String, Object>) vo.getBody();
		Map<String,String> paramMap =new HashMap<String, String>();
		try {
			String user_id = (String) bodyMap.get("message_code");
			String phone = (String) bodyMap.get("phone");
			String password = (String) bodyMap.get("password");
			String messageCode = (String) bodyMap.get("message_code");
			
			paramMap.put("user_id", vo.getHeader().getUser_id().toString());
			paramMap.put("phone", phone);
			paramMap.put("password", MD5.md5(password));

			userDao.thirdBindPhone(paramMap);
			Map<String, Object> dataMap = new HashMap<String, Object>();
			dataMap.put("phone", phone);
			return StringUtil.returnResponseVo(baseConstant.getRightCode(),"绑定手机号成功","","",dataMap);
		} catch (Exception e) {
			log.error("绑定手机号成功异常",e.getMessage());
			e.printStackTrace();
			return StringUtil.returnResponseVo(baseConstant.getErrorCode(),"绑定手机号成功异常","","","");
		}
	}
