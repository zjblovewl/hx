...
<body>
	...
	<script>
	 $(function(){
	    	//查询帖子趋势URL
	    	var queryCardThrend = '<%=basePath%>chartTrend/queryCardThrend.do?rnd=' + Math.random(); 
	    	//导出帖子数据URL
	    	var cardDataExport = '<%=basePath%>chartTrend/queryCardData.do?rnd=' + Math.random();
	    	...
	    	function initData(param){
	    		 $.ajax({  
	    	          ...
	    	          success: function (data) {  
	    	        	var rowsCard1 = data.rows;
	    	        	var rowsCard=eval("("+rowsCard1+")");
	    	           	//帖子趋势 
	    	           	var data_leg = ['发帖总数','拍卖帖总数','交易帖总数']; //图例
	    	          ...
	    	          // 	for(var j=0;j<data_leg.length;j++){
	    	           	   ...
	    	           	   for(var i=0;i<rowsCard.length;i++){
		     	            	data_xAxis.push(rowsCard[i].daytime);	
		     	            	arr.push(rowsCard[i].transactioncount);
		     	            	arr1.push(rowsCard[i].salecount);
		     	            	arr2.push(rowsCard[i].postcount);
	     	           		}
	    	...
	    	$("#build").bind('click',function(){
	    		...
	    		document.getElementById("exportFrame").src = cardDataExport +'&startTime='+startTime+'&endTime='+endTime;
	    	});
	    });
