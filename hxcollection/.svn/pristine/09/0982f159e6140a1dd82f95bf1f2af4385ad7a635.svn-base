...
//            createTab(array_tabs);//创建tab
        ...
        //右键关闭菜单
        $("#Hui_tabs_ul").on("mousedown",'li',function (e) {  
        	var $this = $(this);
        	var indexOF = $this.prevAll().length;
        
        	if (e.which == 3) {
        		var options = {
        				items:[
        				      {text: '关闭标签', onclick: function() {
        				    	  if($this.hasClass('color_theme')){
        				    		  $this.siblings('*:last').addClass('color_theme');
        				    		  //刷新
            				    	  var url = $this.siblings('*:last').attr('data-href');           				    	 
            				    	  $("#iframe").attr('src',url);
        				    	  }       				    	 
        				    	  $this.remove();
        				    	  array_tabs.splice(indexOF,indexOF+1);
        				      }},
        				      {text: '关闭其他标签',onclick: function() {       				    	 
        				    	  //刷新
        				    	  if(!$this.hasClass('color_theme')){
        				    		  var url = $this.attr('data-href');           				    	 
        				    		  $("#iframe").attr('src',url);
        				    	  ...
        				    	  array_tabs.splice(0, indexOF);
        				    	  array_tabs.splice(indexOF+1);
        				    	  $this.addClass('color_theme');
        				    	  $this.siblings().removeClass('color_theme');
        				      }},
        		              ...
        		              {text: '关闭左侧标签',onclick: function() {
        		            	  ...
        		            	  $this.addClass('color_theme');
        				    	  $this.siblings().removeClass('color_theme');
        		            	  //createTab(array_tabs);       				   
        		              }},
        		              {text: '关闭右侧标签', onclick: function() {
        		            	  //刷新
        		            	  if(!$this.hasClass('color_theme')){
        		            		  var url = $this.attr('data-href');           				    	 
        		            		  $("#iframe").attr('src',url);
