 ...
 */
@Service
public class TransactionMobileServiceImpl extends BaseLogger implements TransactionMobileService{
	...
	public ResponseParamVo getGoodsInfoById(RequestParamVo vo){		
		...
		try{						
            Map<String,Object> bodyMap = (Map<String, Object>) vo.getBody();
            //查询藏品详情,记录我的足迹
            auctionMobileService.addMyTrack(bodyMap.get("goods_id").toString(),vo.getHeader().getUser_id(),"1");
            
            //浏览数+1
            transactionMobileMapper.saveGoodsBrowseNum(bodyMap);
            ...
            if(totalOrder > 0){
            	userObj.put("praise_rate", NumberUtil.divide(totalScore, totalOrder,2));
            }else{
	...
	public ResponseParamVo saveGoodsCollection(RequestParamVo vo){
		...
		try{		
            ...
            return StringUtil.returnResponseVo(baseConstant.getRightCode(),resultStr,"","",new JSONObject());               
        }catch (Exception e)
        {
	...
	public ResponseParamVo updateGoodsStatusById(RequestParamVo vo){
		try{		
            ...
            bodyMap.put("uuid", CommonUtils.getUUID());
            transactionMobileMapper.deleteGoodsById(bodyMap);         
            return StringUtil.returnResponseVo(baseConstant.getRightCode(),"更新收藏品或拍卖上架下架状态成功","","",new JSONObject());               
        }catch (Exception e)
        {
            log.error("更新收藏品或拍卖上架下架状态异常",e.getMessage());
            return StringUtil.returnResponseVo(baseConstant.getErrorCode(),"更新收藏品或拍卖上架下架状态异常","","",new JSONObject());
        }
	}
