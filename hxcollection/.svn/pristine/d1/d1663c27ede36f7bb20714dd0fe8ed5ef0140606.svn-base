...
<body>
	...
	<script>
		$(function() {		
			...
			if(json_data.callbackTxt && json_data.callbackTxt == 'callback'){
			...
			$("#lable_icon").val(json_data.icon);
			$("#lable_icon").next().remove();
			 var Icon = '<span class="fa '+json_data.icon+' phone_icon" aria-hidden="true"></span>';
		     $("#lable_icon").after(Icon); 
			//关闭
			...
			$("#lable_icon").bind('click',function(){
				parentOBJ.setWindowDialog(820,430);
				var name = $("#name").val();
				...
				parentOBJ.modalOut('component/awesome_icon.jsp',params);
			});
		});
	</script>
</body>
