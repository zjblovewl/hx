...
<script>
	...
	$("#btnStart").bind('click', function() {
		...
		for(var i=0;i<getSelectRows.length;i++){
			...
			start.push(a);
			/* startId.push(getSelectRows[i].id);
			startName.push(getSelectRows[i].name);
			startClassName.push(getSelectRows[i].classname);
			startExpression.push(getSelectRows[i].expression);
			startStartTime.push(getSelectRows[i].start_time);
			startEndTime.push(getSelectRows[i].end_time); */
		}
		//console.log(startId+startName+startClassName+startExpression+startStartTime+startEndTime)
		$.ajax({
			url:startByIdUrl,
			type:"post",
			data:{datas:JSON.stringify(start)
				/* ids:startId.join(","),
				names:startName.join(","),
				classnames:startClassName.join(","),
				expressions:startExpression.join(","),
				starttimes:startStartTime.join(","),
				endtimes:startEndTime.join(",") */
			},
			success:function(data){
				$("#table").bootstrapTable('refresh');//刷新
			},
